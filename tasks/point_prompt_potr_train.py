...
def main():
    ...
    # Loss
    optimizer_detection = config.OPTIMIZER(list(model.point_detector.parameters()), **config.OPTIMIZER_KWARGS_DETECTION)
    ...
    for epoch in range(1, config.MAX_EPOCHS + 1):
        ...
        train_class_error=AverageMeter()
        train_total_loss = AverageMeter()

        val_cls_loss= AverageMeter()
        val_box_loss=AverageMeter()
        val_class_error=AverageMeter()
        ...
        for iter,batch in enumerate(tqdm(train_loader, desc=f"epoch: {epoch}", disable=not accelerator.is_main_process)):
            with accelerator.accumulate(model):
                ...
                outputs = model.forward_point(input_images)
                ...
                train_cls_loss.update(loss_dict['loss_ce'].item(),images.shape[0])
                train_box_loss.update(loss_dict['loss_bbox'].item(),images.shape[0])
                train_class_error.update(loss_dict['class_error'].item(),images.shape[0])
                train_total_loss.update(losses.item(), images.shape[0])   
        ...
        for iter,batch in enumerate(tqdm(val_loader, desc=f"epoch: {epoch}", disable=not accelerator.is_main_process)):
            with accelerator.accumulate(model):
                ...
                weight_dict = criterion.weight_dict
                losses = sum(loss_dict[k] * weight_dict[k] for k in loss_dict.keys() if k in weight_dict)
                val_cls_loss.update(loss_dict['loss_ce'].item(),images.shape[0])
                val_box_loss.update(loss_dict['loss_bbox'].item(),images.shape[0])
                val_class_error.update(loss_dict['class_error'].item(),images.shape[0])
                val_total_loss.update(losses.item(), images.shape[0])

        print(f'Epoch : {epoch+1}\n'+
            f'Training: train_cls_loss:{train_cls_loss.avg:.4f} train_box_loss:{train_box_loss.avg:.4f} train_class_error:{train_class_error.avg:.4f} train_total_loss:{train_total_loss.avg:.4f} \n'+
            f'Validating: - val_cls_loss:{val_cls_loss.avg:.4f} val_box_loss:{val_box_loss.avg:.4f} val_class_error:{val_class_error.avg:.4f} val_total_loss:{val_total_loss.avg:.4f} \n')
        # After epoch
        # scheduler.step()
